...
describe('resolveChromiumExecutablePath', () => {
  test('prefers the explicit playwright executable path', () => {
    expect(
      resolveChromiumExecutablePath(
        { PLAYWRIGHT_CHROMIUM_EXECUTABLE: '/custom/chromium' } as unknown as NodeJS.ProcessEnv,
        () => false,
      )
  ...
  test('falls back to the system chromium path when it exists', () => {
    expect(resolveChromiumExecutablePath({} as unknown as NodeJS.ProcessEnv, (path) => path === '/usr/bin/chromium-browser'))
      .toBe('/usr/bin/chromium-browser');
  });

  test('returns undefined when no chromium executable is configured', () => {
    expect(resolveChromiumExecutablePath({} as unknown as NodeJS.ProcessEnv, () => false)).toBeUndefined();
  });
});
