...
import { CHECKOUT_DART_OPTIONS, getAllowedCheckoutDarts } from "@/app/lib/score-entry";
...
<GamepadButton
               ...
               disabled={!hydrated || selectedCheckoutDarts === null || isSubmitting}
               onClick={handleDartsOK}
...
<GamepadButton
             name="REDO"
             color="bg-gray-800/80 text-gray-300 ring-white/10 hover:bg-gray-700"
             disabled={!hydrated || !throwHistory.some(playerThrow => playerThrow.status === 'undone') || isSubmitting}
             onClick={handleRedo}
           />
          {items.map((item) => (
            <GamepadButton
