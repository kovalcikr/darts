...
type PrismaTransactionClient = Omit<
  ...
  "$transaction" | "$on" | "$connect" | "$disconnect" | "$use"
>;
...
export function getSyncedMatchLegState(match: {
  ...
  if (
    !Number.isInteger(runTo) ||
    !Number.isInteger(playerALegs) ||
    !Number.isInteger(playerBlegs)
  ) {
    return null;
  }
...
export async function refreshMatchLiveState(
  ...
  const throwCount =
    (playerALegTotals?._count?.id ?? 0) + (playerBLegTotals?._count?.id ?? 0);
  ...
  const getNextActivePlayer = () => {
    ...
    return match.firstPlayer === match.playerAId
      ? match.playerBId
      : match.playerAId;
  };
